...
// Shared Tailwind classes (consider defining these globally or using @apply)
...
export default function LoginForm({ onSuccess, onSwitchToSignup }: LoginFormProps) {
    ...
    // --- Firestore Profile Check/Create Helper ---
    const checkOrCreateUserProfile = async (user: User) => {
        if (!db) {
             console.error("Firestore client (db) not available.");
             // Optionally throw error or display message
             return; // Exit if db is not initialized
        }
        ...
        try {
            ...
            // Decide how to handle this - maybe still allow login but log error
        }
    }
